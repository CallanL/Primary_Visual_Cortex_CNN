...
data_dir = pathlib.Path("../DataGenerationUpdated/UserDataTraining/") #CHANGE THIS DIRECTORY TO LOCATION OF THE DATA
...
for train, test in kfold.split(inputs, targets):
    ...
    class plateuStop(tf.keras.callbacks.Callback):
        ...
        def accuracyPlateu(self):
            ...
            if len(self.accuracies) < 350:          #This number refers to the minimum number of epochs (350)
                return False
            if (max(self.accuracies[-20:]) < max(self.accuracies)):           #This number refers to the number of epochs that need to be the same before platarau 
                return True                                                   #Can change the accurayc to loss and < to a > to change to plateau at loss
            else:
                return False
...
#AGenerates a confucsion matrix
#pred = model.predict(
#    validation_ds,
#    batch_size=4)
